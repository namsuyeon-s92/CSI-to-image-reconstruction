...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.start_h, self.start_w = 15, 20
        # Optimized hidden dimensions for VGA resolution on 24GB RAM
        hidden_dims = [256, 128, 64, 32, 16]
        self.start_dim = hidden_dims[0]
        ...
        # Final upsampling stage to reach 640x480
        modules.append(
            nn.Sequential(
                nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
        ...
        self.decoder_input = nn.Linear(z_dim, self.start_dim * self.start_h * self.start_w)

    def decode(self, z):
        z = self.decoder_input(z)
        z = z.view(-1, self.start_dim, self.start_h, self.start_w)
