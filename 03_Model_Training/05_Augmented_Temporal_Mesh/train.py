...
def train():
    base_dataset = WificamDataset(data_dir, window_size)

    train_idx, val_idx = train_test_split(list(range(len(base_dataset))), test_size=0.1, shuffle=False)

    dataset_train = AugmentedDatasetWrapper(Subset(base_dataset, train_idx), augment=True)
    dataset_val = AugmentedDatasetWrapper(Subset(base_dataset, val_idx), augment=False)
