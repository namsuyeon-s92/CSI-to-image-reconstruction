...
class AugmentedDatasetWrapper(Dataset):
    def __init__(self, dataset, augment=False, noise_std=0.05, mask_max_len=15):
    ...
    def __getitem__(self, index):
        spectrogram, image = self.dataset[index]
        ...
        if self.augment:
            ...
            # # 3. Time Masking (Randomly zero out a block of time frames)
            # if self.mask_max_len > 0 and spectrogram.shape[0] > self.mask_max_len:
            #     mask_len = torch.randint(1, self.mask_max_len, (1,)).item()
            #     mask_start = torch.randint(0, spectrogram.shape[0] - mask_len, (1,)).item()
            #     spectrogram[mask_start:mask_start+mask_len, :] = 0.0
